...
const cleanupCloudinaryImages = async (images) => {
  for (const image of images) {
    ...
    try {
      ...
      console.warn(`Cloudinary cleanup failed for ${image.public_id}:`, error.message);
    }
  }
};
...
const extractPublicIdFromUrl = (url) => {
  ...
  try {
    ...
    while (segments[0] && /^v\d+$/.test(segments[0])) segments.shift();
...
const validateCommonFields = ({ name, description, price, category, stock }) => {
  ...
  if (price === "" || isNaN(price) || price < 0) {
    return { error: "Invalid price", message: "Price must be a valid positive number" };
  }
...
export const addProductWithStableImages = async (req, res) => {
  try {
    ...
    if (error.message?.includes("timed out")) {
      return res.status(504).json({ error: "Request timeout", message: error.message });
    }
...
export const updateProductWithStableImages = async (req, res) => {
  try {
    ...
    if (name !== undefined && (!name || !name.trim())) {
      return res.status(400).json({ error: "Invalid name", message: "Product name cannot be empty" });
    }
    if (description !== undefined && (!description || !description.trim())) {
      return res.status(400).json({ error: "Invalid description", message: "Product description cannot be empty" });
    }
    if (category !== undefined && (!category || !category.trim())) {
      return res.status(400).json({ error: "Invalid category", message: "Product category cannot be empty" });
    }
    if (price !== undefined && (price === "" || isNaN(price) || price < 0)) {
      return res.status(400).json({ error: "Invalid price", message: "Price must be a valid positive number" });
    }
    if (stock !== undefined && (stock === "" || isNaN(stock) || stock < 0)) {
      return res.status(400).json({ error: "Invalid stock", message: "Stock must be a valid non-negative number" });
    }
