...
export const generateInvoice = async (order) => {
  ...
  // ✅ Embed logo — use Khan Medicines logo
  const logoPath = path.join(__dirname, "../assets/khan_medicines_logo.png");
  const logoBuffer = fs.readFileSync(logoPath);
  const logoImage = await pdfDoc.embedPng(logoBuffer);

  const page = pdfDoc.addPage([620, 920]); // wider + taller
  const { height } = page.getSize();

  const margin = 30; // reduced for more usable width
  let y = height - margin;
  ...
  // === FOOTER ===
  y -= 40;
  page.drawText("For Khan Medicines (Chemist and Druggist),", {
    x: 40,
    y,
    size: 10,
    font: customFont,
