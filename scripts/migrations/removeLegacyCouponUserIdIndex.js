...
async function migrate() {
  try {
    ...
    if (!hasLegacyIndex) {
      console.log(
        `[migration] Legacy index "${INDEX_NAME}" does not exist. No changes required.`
      );
      return;
    }
    ...
    console.log(
      `[migration] Dropped legacy index "${INDEX_NAME}" from "${COLLECTION_NAME}".`
    );
  } finally {
